...
// القائمة الرئيسية للجميع (تتيح لك الطلب كأي زبون، وتم إضافة زر خاص للإدارة إذا كنت أنت المدير)
const getMainKeyboard = (userId: number) => {
  ...
  // إذا كان المستخدم هو المدير، نضيف له زراً مباشراً للوحة التحكم في القائمة
...
// أمر خاص لفتح لوحة التحكم للمدير في أي وقت
...
bot.callbackQuery("admin_panel_home", async (ctx) => {
  ...
  await ctx.editMessageText("👑 **لوحة تحكم الإدارة:**", { reply_markup: adminPanelKeyboard, parse_mode: "Markdown" });
});
...
bot.callbackQuery("back_to_main", async (ctx) => {
  ...
  await ctx.editMessageText(welcomeText, { reply_markup: getMainKeyboard(userId) });
});
...
// معالجة خطوات الطلب والأزرار التفاعلية
bot.on("callback_query:data", async (ctx) => {
  ...
  if (data.startsWith("srv_")) {
    ...
    // إشعار المدير (حتى لو طلبت أنت كمدير، سيصلك الإشعار)
    try {
      const adminMsg = 
...
bot.on("message:text", async (ctx) => {
  ...
  // 2. خطوات الزبون أو المدير أثناء الطلب
  const session = userSessions[userId];
  ...
  // 3. تحويل استفسارات الزبائن العادية إليك (إذا راسلك شخص آخر غيرك)
  if (userId !== ADMIN_CHAT_ID) {
    const clientName = ctx.from?.first_name || "عميل";
...
bot.start();
console.log("تم تشغيل البوت بنظام الطلب الشامل ولوحة الإدارة...");

const server = createServer((req, res) => {
  res.writeHead(200, { "Content-Type": "text/plain" });
  res.end("Design Bot is running 24/7!");
});
