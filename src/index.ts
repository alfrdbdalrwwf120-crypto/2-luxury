...
bot.callbackQuery("cmd_portfolio", async (ctx: Context) => {
  ...
  for (const item of portfolioItems) {
    try {
      ...
      console.error("خطأ في إرسال نموذج:", error);
    }
  }
});
...
bot.callbackQuery("cmd_contact", async (ctx: Context) => {
  ...
  await ctx.reply(
    "📞 أهلاً بك!\n\n" +
    "اكتب استفسارك أو التفاصيل هنا مباشرة، وسيتم تحويلها فوراً للمصمم للرد عليك."
  );
});
...
bot.callbackQuery("cmd_my_orders", async (ctx: Context) => {
  const userId = ctx.from ? ctx.from.id : 0;
...
bot.on("callback_query:data", async (ctx: Context) => {
  ...
  if (
    ...
    if (data.startsWith("srv_")) {
      const srvIndex = parseInt(
        data.replace("srv_", ""),
        10
      );

      const service = servicesList[srvIndex];
      ...
      const pkgIndex = parseInt(
        data.replace("pkg_", ""),
        10
      );

      const pkg = packagesList[pkgIndex];
  ...
  if (
    ...
    const adminOrderKeyboard = new InlineKeyboard()
      .text(
        "⏳ قيد التجهيز",
...
bot.on("message:photo", async (ctx: Context) => {
  ...
  // إرسال صورة للعميل بالرد على رسالة الطلب
  if (
    userId === 
