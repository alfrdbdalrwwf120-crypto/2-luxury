...
const reviews: Review[] = [
  ...
  },
  {
    userName: "سارة م.",
    rating: "⭐⭐⭐⭐⭐",
    text: "هوية بصرية متكاملة وفاخرة، شكراً للمحترف للتصميم."
  }
];
...
function getStatusKeyboard(orderId: number): InlineKeyboard {
  return new InlineKeyboard()
    .text("⏳ قيد التجهيز", `st_process_${orderId}`)
    .text("✅ إنجاز", `st_done_${orderId}`)
    .text("❌ إلغاء", `st_cancel_${orderId}`);
}
...
bot.callbackQuery("cmd_services", async (ctx) => {
  ...
  for (const service of servicesList) {
    msg += `${service.name} : ${service.price}\n`;
  }
...
bot.callbackQuery("admin_add_portfolio", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "يمكنك كتابة الوصف في خانة Caption."
  );
});
...
bot.callbackQuery("cmd_contact", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "وسيتم تحويلها للمصمم للرد عليك."
  );
});
...
bot.callbackQuery("cmd_help", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "متخصصون في صناعة الهويات البصرية والشعارات " +
    "والتصاميم المبتكرة بأعلى معايير الفخامة والاحترافية."
  );
});
...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (data.startsWith("rate_")) {
    ...
    await ctx.reply(
      "✍️ شكراً لك!\n\n" +
      "الآن أرسل في رسالة نصية رأيك أو تعليقك على الخدمة."
    );
  ...
  if (
    ...
    if (data.startsWith("srv_")) {
      ...
      if (servicesList[index]) {
        selectedName = servicesList[index].name;
        selectedPrice = servicesList[index].price;
      } else {
    ...
    if (data.startsWith("pkg_")) {
      ...
      if (packagesList[index]) {
        selectedName = packagesList[index].name;
        selectedPrice = packagesList[index].price;
      } else {
  ...
  if (
    ...
    const keyboard = new InlineKeyboard()
      .text("📎 نعم، سأرسل ملفات", "file_yes")
      .text("➡️ لا يوجد", "file_no");

    await ctx.reply(
      "الخطوة 5: هل لديك شعار قديم أو صور ومراجع تحب إرفاقها؟",
      {
        reply_markup: keyboard
      }
    );
  ...
  if (
    ...
    const adminMsg =
      ...
      `📎 المرفقات: ${cleanText(newOrder.details.files || "لا توجد")}\n` +
    ...
    try {
      await bot.api.sendMessage(
        ...
        adminMsg,
        {
          reply_markup: getStatusKeyboard(newOrderId)
        }
      );
    } catch (error) {
...
bot.on("message:photo", async (ctx) => {
  ...
  if (
    ...
    if (photos.length > 0) {
      ...
      const caption =
        ctx.message.caption ||
        "🎨 نموذج تصميم جديد من المحترف للتصميم";
      ...
      await ctx.reply(
        "✅ تم إضافة النموذج بنجاح إلى معرض الأعمال!",
