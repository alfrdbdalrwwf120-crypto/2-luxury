...
// الـ ID الخاص بك لتلقي الطلبات فوراً
...
bot.callbackQuery(/color_/, async (ctx) => {
  ...
  session.data.colors = ctx.callbackQuery.data === "color_yes" ? "نعم، سيتم تحديدها" : "لا، اتركها للمصمم";
  session.step = 5;
...
bot.callbackQuery(/file_/, async (ctx) => {
  ...
  session.data.files = ctx.callbackQuery.data === "file_yes" ? "سيتم إرسال الملفات" : "لا توجد ملفات";
  ...
  // 1. إرسال إشعار فوري لك أنت (المدير) على محادثتك الشخصية
  try {
    const adminMsg = 
