...
const userSessions: Record<number, SessionData> = {};
...
// دالة لتنظيف الرموز الخاصة بالـ Markdown ومنع انهيار الإرسال
...
bot.command("admin", async (ctx) => {
  ...
  await ctx.reply("👑 **لوحة تحكم إدارة المحترف للتصميم:**", { reply_markup: adminPanelKeyboard, parse_mode: "Markdown" });
});

bot.callbackQuery("admin_panel_home", async (ctx) => {
  if (ctx.from?.id !== ADMIN_CHAT_ID) return;
  await ctx.answerCallbackQuery();
  await ctx.editMessageText("👑 **لوحة تحكم إدارة المحترف للتصميم:**", { reply_markup: adminPanelKeyboard, parse_mode: "Markdown" });
});

bot.callbackQuery("back_to_main", async (ctx) => {
  const userId = ctx.from?.id || 0;
  await ctx.answerCallbackQuery();
  await ctx.editMessageText("✨ **القائمة الرئيسية - المحترف للتصميم:**", { reply_markup: getMainKeyboard(userId), parse_mode: "Markdown" });
});
...
bot.callbackQuery("cmd_get_files", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "أرسل **رقم الطلب** الخاص بك في رسالة (مثال: `1234` أو `#1234`) وسنقوم بتجهيز الصور والملفات وإرسالها لك مباشرة!",
    { parse_mode: "Markdown" }
  );
});
...
bot.callbackQuery("cmd_portfolio", async (ctx) => {
  ...
  for (const item of portfolioItems) {
    try {
      await ctx.replyWithPhoto(item.fileId, { caption: item.caption, parse_mode: "Markdown" });
    } catch (e) {}
  }
});
...
bot.callbackQuery("admin_add_portfolio", async (ctx) => {
  ...
  await ctx.reply("🖼️ **إضافة تصميم جديد للمعرض:**\n\nقم بإرسال **الصورة** الآن مباشرة، واكتب التفاصيل/الوصف في خانة الشرح (Caption) الخاصة بالصورة.");
});
...
bot.callbackQuery("cmd_reviews", async (ctx) => {
  ...
  reviews.forEach(r => {
    msg += `👤 **${escapeMarkdown(r.userName)}** - ${r.rating}\n💬 "${escapeMarkdown(r.text)}"\n------------------\n`;
  });
  await ctx.reply(msg, { parse_mode: "Markdown" });
});
...
bot.callbackQuery("admin_list_orders", async (ctx) => {
  ...
  if (orders.length === 0) {
    ...
    orders.forEach(o => {
      msg += `🔹 طلب #${o.id}\n👤 العميل: ${escapeMarkdown(o.userName)}\n🎨 الخدمة: ${o.serviceName}\n📌 المشروع: ${escapeMarkdown(o.details.projectName || "غير محدد")}\nالحالة: ${o.status}\n------------------\n`;
    });
    await ctx.reply(msg, { parse_mode: "Markdown" });
  }
});
...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (data.startsWith("srv_") || data.startsWith("pkg_")) {
    ...
    if (data.startsWith("srv_")) {
      ...
      selectedName = servicesList[srvIndex]?.name || "خدمة تصميم";
      selectedPrice = servicesList[srvIndex]?.price || "";
    } else {
      const pkgIndex = parseInt(data.replace("pkg_", ""));
      selectedName = packagesList[pkgIndex]?.name || "باقة تصميم";
    ...
    try {
      const adminMsg = 
        ...
        `👤 العميل: ${escapeMarkdown(clientName)} (${escapeMarkdown(clientUsername)})\n` +
        `🆔 معرف المستخدم: \`${userId}\`\n` +
        `🎨 الخدمة/الباقة: ${newOrder.serviceName}\n` +
        `📌 اسم المشروع: ${escapeMarkdown(newOrder.details.projectName || "غير محدد")}\n` +
        `🏷️ المجال: ${escapeMarkdown(newOrder.details.projectField || "غير محدد")}\n` +
        `💡 الفكرة: ${escapeMarkdown(newOrder.details.idea || "غير محدد")}\n` +
...
bot.on("message:photo", async (ctx) => {
  ...
  if (userId === ADMIN_CHAT_ID && ctx.message.reply_to_message) {
    ...
    if (extractedId) {
      ...
      try {
        await bot.api.sendPhoto(targetUserId, fileId, { caption: caption, parse_mode: "Markdown" });
...
bot.on("message:text", async (ctx) => {
  ...
  if (session && session.step === 70) {
    ...
    try {
      const adminNotice = 
        ...
        `👤 العميل: ${escapeMarkdown(clientName)} (${escapeMarkdown(clientUsername)})\n` +
        `🆔 معرف المستخدم: \`${userId}\`\n` +
        `🔢 رقم الطلب المدخل: **${escapeMarkdown(orderNum)}**\n\n` +
        `_💡 قم بعمل Reply على هذه الرسالة وأرفق (صورة أو ملف PDF/ZIP) لتسليمه للعميل فوراً._`;
  ...
  if (session && session.step === 50) {
    ...
    try {
      ...
      await bot.api.forwardMessage(ADMIN_CHAT_ID, ctx.chat.id, ctx.message.message_id);
