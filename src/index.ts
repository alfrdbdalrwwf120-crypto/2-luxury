...
bot.callbackQuery("admin_list_orders", async (ctx) => {
  ...
  if (orders.length === 0) {
    ...
    orders.forEach(o => {
      msg += `🔹 طلب #${o.id}\n👤 العميل: ${o.userName}\n🎨 الخدمة: ${o.serviceName}\n📌 المشروع: ${o.details.projectName || "غير محدد"}\nالحالة: ${o.status}\n------------------\n`;
    });
    await ctx.reply(msg, { parse_mode: "Markdown" });
  }
});
  ...
  if (data.startsWith("srv_") || data.startsWith("pkg_")) {
    ...
    try {
      const adminMsg = 
        ...
        `📌 اسم المشروع: ${newOrder.details.projectName || "غير محدد"}\n` +
        `🏷️ المجال: ${newOrder.details.projectField || "غير محدد"}\n` +
        `💡 الفكرة: ${newOrder.details.idea || "غير محدد"}\n` +
...
// 📸 تسليم صور حقيقية (PNG / JPG) من الإدارة للعميل مباشرة
bot.on("message:photo", async (ctx) => {
  const userId = ctx.from?.id || 0;
  const session = userSessions[userId];

  // أ) إضافة تصميم للمعرض
  ...
  // ب) إرسال وتسليم صورة حقيقية للعميل عبر Reply
...
// 📁 تسليم ملفات وحزم حقيقية (PDF / ZIP / RAR / PSD) للعميل عبر Reply
