...
// دالة لتنظيف النوافذ والرموز لمنع انهيار الرسائل
...
// صيد الأخطاء ومنع توقف البوت
bot.catch((err) => {
  console.error("حدث خطأ في البوت ولكن تم استدراكه بنجاح:", err.error);
});
...
bot.on("message:photo", async (ctx) => {
  ...
  if (userId === ADMIN_CHAT_ID && ctx.message.reply_to_message) {
    ...
    if (extractedId) {
      ...
      await ctx.reply("⚠️ قم بعمل رد (Reply) على الرسالة التي تحتوي على ID العميل لتوصيل الصورة له.");
    }
  }
});

// التعامل مع الملفات المستندية
bot.on("message:document", async (ctx) => {
  ...
  if (userId === ADMIN_CHAT_ID && ctx.message.reply_to_message) {
    ...
    if (extractedId) {
      ...
      await ctx.reply("⚠️ قم بعمل رد (Reply) على الرسالة التي تحتوي على ID العميل لتوصيل الملف له.");
    }
  }
});
